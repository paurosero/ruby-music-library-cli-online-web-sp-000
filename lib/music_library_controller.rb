...
class MusicLibraryController
 ...
 def play_song
   ...
   if (1..Song.all.length).include?(gets.strip.to_i)
     song_array = Song.all.sort {|a, b| a.name <=> b.name}[gets.strip.to_i - 1]
   end
   puts "Playing #{song_array.name} by #{song_array.artist.name}" if song_array
 end
